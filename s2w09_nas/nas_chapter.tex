\chapter{Network-Attached Storage}
\label{ch:network-attached-storage}
...
{1.0}{[}htbp{]}{l l l X} \textbf{Protocol} \& ~ \& \textbf{Port} \&
\textbf{Remarks}\\
...
Netware Core Protocol \& NCP \& 524 \& Like SMB has support for print
and other sharing as well.\\
Points to note:
...
\subsection{Access pattern}\label{access-pattern}
...
\section{Provisioning}\label{provisioning}

\subsection{General-purpose server}\label{general-purpose-server}
...
\subsection{Storage appliance}\label{storage-appliance}
...
\subsection{NAS gateway}\label{nas-gateway}
