\chapter{Magnetic disks}
\label{ch:magnetic-disks}

\section{Components}
\label{sec:components}

%\includegraphics{hard_drive_components_3d.svg}

Hard drive components

\begin{itemize}
\item
  Hard disk drives encased with controller board and power / data
  connectors.
\item
  1 or more aluminimum platters mounted on spindle separated by a small
  gap.
\item
  Spindle attached to motor spins platters at constant speed. (Usually
...
%\includegraphics{disk_geometry.svg}
...
\section{Logical block addressing}\label{logical-block-addressing}
...
\subsection{Converting CHS to LBA}\label{converting-chs-to-lba}
...
\section{Zoned bit recording}\label{zoned-bit-recording}

%\includegraphics{zoned_bit_recording.png}
...
\begin{itemize}
\item
  Cylinders are grouped into zones based on their distance from the
  spindle.
\item
...
\subsection{Converting from advertised to actual
capacity}\label{converting-from-advertised-to-actual-capacity}
...
where \texttt{x} depends on the units in question:
...
\subsection{Converting from actual to advertised
capacity}\label{converting-from-actual-to-advertised-capacity}
...
where {\emph{x}} depends on the units in question as above.
