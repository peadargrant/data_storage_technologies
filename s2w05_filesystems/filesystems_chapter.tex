...
\section{Disk-based filesystems}\label{disk-based-filesystems}
...
\subsection{Common filesystems}\label{common-filesystems}
...
\section{Journalling}\label{journalling}
...
Journalling}\label{alternatives-to-journalling}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# review help text and manpage}
\ExtensionTok{mkfs} \NormalTok{-h }
\FunctionTok{man} \NormalTok{mkfs}

\CommentTok{# check where mkfs is located (usually /sbin)}
\FunctionTok{which} \NormalTok{mkfs}

\CommentTok{# from that, find the available filesystems}
\FunctionTok{ls} \NormalTok{-l /sbin }\KeywordTok{|} \FunctionTok{grep} \NormalTok{mkfs}

\CommentTok{# creating a filesystem (aka formatting)}
\CommentTok{# e.g. make an ext4 on /dev/sdb1}
\ExtensionTok{mkfs} \NormalTok{-t ext4 /dev/sdb1}
\end{Highlighting}
\end{Shaded}
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# review mount command}
\FunctionTok{mount} \NormalTok{-h  }\CommentTok{#inbuilt help}
\FunctionTok{man} \NormalTok{mount  }\CommentTok{#man page}

\CommentTok{# create the mountpoint (-p allows it to already exist)}
\FunctionTok{mkdir} \NormalTok{-p /mnt/data }

\CommentTok{# issue the mount}
\FunctionTok{mount} \NormalTok{-t ext4 /dev/sdb1 /mnt/data}

\CommentTok{# viewing mounted systems}
\FunctionTok{mount}
\end{Highlighting}
\end{Shaded}
\end{verbatim}
...
\section{Virtual filesystems}\label{virtual-filesystems}
